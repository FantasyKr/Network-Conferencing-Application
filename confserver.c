...
int main(int argc, char *argv[]) {
 ...
 // 	for (int i = 0; i < clients->size; i++){
	//  	if (clients->client_list[i] > livesdmax){
	// 	 	livesdmax = clients->client_list[i];
	// 	}
	// 	FD_SET(clients->client_list[i], &livesdset);
 // 	}
//----------------------------------------------
	...
	while (1) {
		...
		for (frsock = 3; frsock <= livesdmax; frsock++) {
			...
			if ( FD_ISSET(frsock, &rst)!=0/* FILL HERE: message from client 'frsock'? */ ) {
		...
		if (mflag == 0 && FD_ISSET(frsock, &livesdset)==0 /* FILL HERE: connect request from a new client? */ ) {
					//printf("Jiajia\n");
			/*
			 FILL HERE:
